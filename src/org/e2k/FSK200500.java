...
public class FSK200500 extends MFSK {
	...
	private String syncToneHunt (CircularDataBuffer circBuf,WaveData waveData)	{
		...
		String line;
		if (theApp.isSoundCardInput()==true)  line=theApp.getTimeStamp()+" FSK200/500 Sync Tones Found (Correcting by "+Integer.toString(correctionValue)+" Hz) sync tone difference "+Integer.toString(difference)+" Hz";
		else line=theApp.getTimeStamp()+" FSK200/500 Sync Tones Found (Correcting by "+Integer.toString(correctionValue)+" Hz) at "+Long.toString(sampleCount)+" sync tone difference "+Integer.toString(difference)+" Hz";
		return line;
	}
